...
"""
    ...
    # Use --no-scripts on both paths so the post-autoload-dump hook (php artisan
    # package:discover) is NOT triggered here — artisan doesn't exist until phase 6.
    run(f"cd {APP_DIR} && composer update --no-interaction --no-audit --no-scripts 2>&1 || "
        f"composer update --no-interaction --ignore-platform-reqs --no-scripts 2>&1")
    run(f"cd {APP_DIR} && composer dump-autoload --no-scripts")
...
def phase7_finalize():
    print("\n=== Phase 7: Finalize ===")
    commands = [
        # Run without --no-scripts now that artisan exists (written in phase 6).
        # This triggers the post-autoload-dump hook (php artisan package:discover)
        # so all Laravel service providers are properly registered.
        f"cd {APP_DIR} && composer dump-autoload --optimize",
        f"cd {APP_DIR} && php artisan package:discover --ansi",
